defmodule Advent.Day8 do
  ...
  defp parse_line(line) do
    data = Regex.named_captures(~r/(?<a>\w+) (?<b>inc|dec) (?<c>-*\d+) if (?<d>\w+) (?<e>[><=!]+) (?<f>-*\d+)/, line)
    %{register: data["a"], operation: data["b"], value: String.to_integer(data["c"]),
  ...
  defp apply_instruction(instruction, data) do
    ...
    case condition_applies?(instruction.condition, data) do
      true ->
        Map.update!(data, instruction.register, &(apply_operation(&1, instruction.operation, instruction.value)))
      false -> data
    end
  end
